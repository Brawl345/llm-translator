...
- **Content Script** (`entrypoints/content.ts`): mounts the Vue modal inside a shadow root via `createShadowRootUi`
  (`cssInjectionMode: 'ui'`); feeds runtime messages into the shared store.
...
2. Background validates input length against the context limit (4 chars/token estimate).
...
- **Input Validation**: combined system prompt + user text is checked against the context limit (`lib/openai.ts`).
